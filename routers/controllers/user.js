...
const login = (req, res) => {
  ...
  usermodel
    ...
    .then(async (result) => {
      if (result) {
        if (email.toLowerCase() === result.email && result.isDeleted == false) {
...
const useraccount = async (req, res) => {
  ...
  try {
    user.findByIdAndUpdate(req.params.id, { idDeleted: true });
...
const activateAccount = (req, res) => {
  ...
  jwt.verify(token, SECRET_KEY, (err, resul) => {
    ...
    usermodel
      .findOne({ _id: resul._userid, email: req.params.email })
      .exec((err, user) => {
        if (user) {
          return res
            .status(400)
...
const resetPassword = (req, res) => {
  ...
  if (newPass.match(/^(?=.*\d)(?=.*[a-z])(?=.*[A-Z])(?=.*[a-zA-Z]).{7,}$/)) {
    if (resetLink) {
      jwt.verify(
        resetLink,
